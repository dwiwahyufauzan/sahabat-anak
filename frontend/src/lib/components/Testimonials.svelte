...
<div id="testimonials-section" class="py-20 bg-primary/5 dark:bg-gray-900 relative overflow-hidden">
    <!-- Decoration -->
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <h2 class="font-display text-3xl font-bold text-center mb-16 text-gray-900 dark:text-white {visible ? 'animate-fade-in-up' : 'opacity-0'}">
            Cerita Mereka
        </h2>
        
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
            {#each testimonials as testimonial, index (testimonial.id)}
                <div class="bg-white dark:bg-gray-800 p-6 sm:p-8 lg:p-10 rounded-2xl sm:rounded-[2.5rem] shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-2 relative {visible ? 'animate-scale-in' : 'opacity-0'}" style="animation-delay: {index * 0.2}s;">
                    <div class="text-secondary mb-4 sm:mb-6">
                        <span class="material-symbols-outlined text-3xl sm:text-4xl">format_quote</span>
                    </div>
                    <p class="text-base sm:text-lg md:text-xl font-medium text-gray-700 dark:text-gray-300 mb-6 sm:mb-8 leading-relaxed">
                        "{testimonial.quote}"
                    </p>
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h4 class="font-bold text-gray-900 dark:text-white font-display">
                                {testimonial.name}
                            </h4>
