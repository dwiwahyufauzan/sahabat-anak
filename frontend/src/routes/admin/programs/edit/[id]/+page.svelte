<script lang="ts">
  ...
  onMount(async () => {
    try {
      const programId = parseInt($page.params.id || '0');
      const program = await adminApi.programs.getById(programId);
      
      // Populate formData with loaded program data
      formData = {
        slug: program.slug || '',
        title: program.title || '',
        ...
        categoryColor: program.categoryColor || 'blue',
        icon: program.icon || 'menu_book',
        image: program.image || '',
        ...
        scheduleFrequency: program.scheduleFrequency || '',
        scheduleDuration: program.scheduleDuration || '',
      ...
      error = 'Gagal memuat data program. Silakan coba lagi.';
      loadingData = false;
    }
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await adminApi.programs.update(parseInt($page.params.id || '0'), submitData);
      modalType = 'success';
      modalTitle = 'Berhasil!';
      modalMessage = 'Program berhasil diperbarui!';
      showModal = true;
      setTimeout(() => goto('/admin/programs'), 1500);
    } catch (err) {
      console.error('Error updating program:', err);
      error = err instanceof Error ? err.message : 'Gagal memperbarui program. Silakan coba lagi.';
    } finally {
      loading = false;
    }
  };
...
<svelte:head>
  <title>Edit Program - Admin Panel</title>
</svelte:head>
...
{#if loadingData}
  <div class="flex items-center justify-center min-h-screen">
    <div class="text-center">
      ...
      <p class="text-gray-600">Memuat data program...</p>
    </div>
  </div>
{:else}
  <div>
      ...
      <form onsubmit={(e) => { e.preventDefault(); handleSubmit(); }} class="space-y-6">
        ...
        <div>
          <label for="image" class="block text-sm font-medium text-gray-700 mb-2">
            Upload Gambar Thumbnail {imagePreview ? '(Opsional - gambar saat ini akan dipertahankan jika tidak diubah)' : '*'}
          </label>
          <input
            id="image"
            type="file"
