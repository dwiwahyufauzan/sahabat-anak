<script lang="ts">
  ...
  let programId = parseInt($page.params.id || '0');
  ...
  onMount(async () => {
    try {
      const programs = await adminApi.programs.getAll();
  ...
  const handleSubmit = async () => {
    ...
    try {
      await adminApi.programs.update(programId, formData);
      goto('/admin/programs');
    } catch (err) {
    ...
    <div class="bg-white rounded-lg shadow p-8 max-w-3xl">
      <form onsubmit={(e) => { e.preventDefault(); handleSubmit(); }} class="space-y-6">
        ...
        <div class="flex gap-4 pt-4">
          ...
          >
            {saving ? 'Menyimpan...' : 'Update Program'}
          </button>
          <a
            href="/admin/programs"
